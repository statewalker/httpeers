 ...
 * `TooManyFacts` instead of becoming a denial of service. `rules.test.ts`
 * proves that.
 *
 * The shape (snake_case, microseconds) is kept from the wasm era because it is
 * public API; `ENGINE_LIMITS` is what the engine takes. Unlike the wasm build,
 * `max_time_micro` is real here: a `Timeout` is a measured wall clock, never a
 * spurious report, so nothing retries it.
 */
 ...
 * peerId.
 */
function signingKeyFor(seed: Uint8Array): Uint8Array {
  return seed;
}
 ...
 */
async function rootKeysFor(
  issuer: string,
  keys: IssuerKeys | undefined,
...
/**
 * A no-op, kept because it is public API. It absorbed the wasm build's
 * one-time instantiation cost and its spurious first-call `Timeout`
 * (prototype 10's finding F2); a TypeScript engine has neither.
 */
...
export async function verifyToken(token: string, options: VerifyTokenOptions): Promise<MeshClaims> {
  ...
  let verified: VerifiedBiscuit | undefined;
