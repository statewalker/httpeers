...
describe("parseArgs", () => {
  it("defaults to a full run", () => {
    ...
    expect(args.llamaswap).toBe(false);
  });
...
describe("run", () => {
  ...
  it("writes llamaswap/config.yaml and routes every local model's LiteLLM entry through llamaswap when --llamaswap is set", async () => {
    ...
    expect(files.has("/a/llamaswap/config.yaml")).toBe(true);
    expect(report.wrote).toContain("/a/llamaswap/config.yaml");
    const llamaswapConfig = files.get("/a/llamaswap/config.yaml") as string;
    expect(llamaswapConfig.startsWith("# GENERATED by bin/prepare.sh")).toBe(true);
    expect(llamaswapConfig).toMatch(/small-a:/);
    expect(llamaswapConfig).toMatch(/small-b:/);

    // Every local model's api_base points at the one llamaswap service --
    // never at an individual llamacpp-<id> one -- and compose.models.yml
    // (still generated -- see compose.llamaswap.yml's own header on why) is
    // simply never read by it.
    const litellm = files.get("/a/litellm/config.local.yaml") as string;
    expect(litellm).toContain("http://llamaswap:8080/v1");
    expect(litellm).not.toContain(`${serviceNameOf("small-a")}:8080`);
    expect(litellm).not.toContain(`${serviceNameOf("small-b")}:8080`);

    // compose.models.yml is unconditionally written either way (prepare does
    // not special-case it away under --llamaswap).
    expect(files.has("/a/compose.models.yml")).toBe(true);
  });

  it("never writes llamaswap/config.yaml without --llamaswap", async () => {
    const { io, files } = fakeIo({ ...GOOD_RAW_PROBE, "/a/models.json": MODELS_JSON });
    const args = parseArgs([
      "--raw-probe",
      "/probe",
    ...
    await run(args, io);
    expect(files.has("/a/llamaswap/config.yaml")).toBe(false);
    const litellm = files.get("/a/litellm/config.local.yaml") as string;
    expect(litellm).toContain(`${serviceNameOf("small-a")}:8080`);
  });
